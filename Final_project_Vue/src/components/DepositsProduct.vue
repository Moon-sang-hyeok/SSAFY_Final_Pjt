...
export default {
  ...
  methods: {
    async fetchProducts() {
      this.loading = true;
      try {
        const response = await axios.get("http://127.0.0.1:8000/products/api/deposits-comparison/");
        ...
        <li v-for="(product, index) in paginatedProducts" :key="index">
          <div>
            <h3>{{ product.fin_prdt_nm }}
                <router-link :to="`/deposits-comparison/${product.id}`" class='button'>상세보기</router-link>
            </h3>
